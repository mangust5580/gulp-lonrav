...
- **`build:fast`** — более быстрая production-сборка (использует профиль `basic`; пропускает «тяжёлые» оптимизации, см. [Профили](#профили) ниже)
- **`buildFast`** — gulp-задача, которую вызывает `build:fast`
...
Code splitting (чанки для dynamic import) поддерживается, но **по умолчанию выключен** (быстрее сборка, проще артефакты).  
Включается через `config/scripts.js` / user config override: `scripts.splitting: true`.
...
- **`config/features.js`** — включает и выключает опциональные модули (фавиконки, i18n, SVG-спрайт и др.).  
  Например, для поддержки мультиязычности установите `i18n.enabled = true` и добавьте файлы переводов в `src/data/locales/`.
...
1. Создайте файлы переводов в `src/data/locales/`, например `en.json` и `ru.json`.  
...
Подробности — на странице `/docs.html` после запуска `npm run dev`.
...
Профили переключают группы фич (компромисс: время сборки vs. качество/проверки).

- **`full`** (по умолчанию) — включены все оптимизации и проверки.
- **`basic`** — выключает тяжёлые/опциональные шаги:
  - `favicons`
  - `svgSprite`
  - `static`
  - `media` (copy/transcode)
  - `versioning` (rev-hash)
  - `reports.bundleSizes`

Задать профиль можно через env или CLI:
...
- Встроенные профили определены в `config/features.js` (объект `profiles`).
- Для проектных оверрайдов создайте `config.js` в корне и переопределяйте `features` (мерджится глубоко во время запуска).
...
После запуска `npm run dev` откройте `/docs.html` в браузере, чтобы прочитать:
