...
- **`build:fast`** — faster build (skips heavy optimisations, see [Profiles](#profiles) below)
...
You can adjust rules and strictness via `.eslintrc` and `.stylelintrc` in the project root.
...
- **`config/features.js`** — enable/disable optional modules (e.g. favicons, i18n, SVG sprites).  
  Example: to enable localisation support, set `features.i18n.enabled = true` and provide translation files in `src/locales/`.
...
1. Create translation JSON files in `src/locales/`, for example `en.json` and `ru.json`.  
