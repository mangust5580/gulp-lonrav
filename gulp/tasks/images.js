...
async function processRasterDev(fileAbs, relFromBase, cfg) {
  ...
  const writeDerived = async (targetBaseName, resizeScale) => {
    ...
    if (doResize) {
      if (ext === '.jpg' || ext === '.jpeg') {
        ...
        await img.clone().png({ compressionLevel: 6 }).toFile(
          path.join(destDir, `${targetBaseName}${ext}`),
        )
      } else if (ext === '.webp') {
  ...
  await writeDerived(baseName, null)
...
export const imagesTask = async () => {
  try {
    ...
    if (!env.isProd) {
      ...
      const needDerived = needDerivedByMode && needDerivedByCfg
      ...
      await asyncPool(cfg.concurrency ?? 16, files, async abs => {
        ...
        await processRasterDev(abs, rel, localCfg)
      })
