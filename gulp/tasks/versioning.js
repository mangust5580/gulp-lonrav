...
export const versioningTask = async () => {
  ...
  const rewriteGlobsAbs = (versioning.rewriteGlobs || []).map(g => path.join(paths.out, g))
  const manifestPath = path.join(paths.out, versioning.manifestName)

  await runStream(
    gulp
      .src(assetsGlobAbs, { base: paths.out, allowEmpty: true, encoding: false })
      .pipe(rev())
  ...
  await runStream(
    gulp
      .src(rewriteGlobsAbs, { allowEmpty: true })
