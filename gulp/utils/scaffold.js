// gulp/utils/scaffold.js
// Minimal dev-only scaffold to keep the build reliable and the structure predictable.
// Controlled fully by config/scaffold.js (no extra CLI flags).
...
const isCi = (() => {
  const v = String(process.env.CI || '').trim().toLowerCase()
...
const failMissing = (what, p) => {
  ...
  throw new Error(`[scaffold] Missing ${what}: ${rel}\n` +
...
export const scaffoldTask = async () => {
  ...
  const baseDirs = [
    ...
    // Favicons are optional. Only require/create their directory when the module is enabled.
    ...(features.favicons?.enabled === false ? [] : [paths.assets.faviconsBase]),
    // NOTE: src/static is optional. We do not require it in strict mode.
    // The static pipeline is a no-op when the directory is missing.
  ]
  ...
  for (const d of baseDirs) {
    if (auto) {
      ...
      if (!statSafe(d)?.isDirectory()) missing.push({ what: 'directory', path: d })
    }
  }

  // Minimal entrypoints (so pipelines do not crash on empty projects)
  const entries = [
    {
      what: 'scripts entry',
  ...
  for (const e of entries) {
    if (auto) {
      ...
      if (!statSafe(e.path)?.isFile()) missing.push({ what: e.what, path: e.path })
    }
  }

  if (!auto && missing.length) {
    // Strict mode for build/CI or when explicitly configured.
    // Fail on first missing item to keep the error readable.
    failMissing(missing[0].what, missing[0].path)
  }

  if (created.length) {
    // logger already prefixes with [scaffold]
